...
export function generatePDF(analysis: ContractAnalysis, lang: "fr" | "en") {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    const footerText = lang === "fr" 
      ? `ThusL8 • Audit de haute précision • Généré le ${new Date().toLocaleDateString("fr-FR")}`
      : `ThusL8 • High-precision Audit Engine • Generated on ${new Date().toLocaleDateString()}`;
